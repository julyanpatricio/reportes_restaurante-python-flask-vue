<template>
  <div v-if="salesByProducts">
    ...
    <md-table>
      ...
      <md-table-row  v-for="(value, key) in salesByProducts" :key="key">
        <md-table-cell>{{key}}</md-table-cell>
        <md-table-cell>{{ value['category'] }}</md-table-cell>
...
export default {
  computed: {
    ...mapState(['salesByProducts', 'dateMinSelected', 'dateMaxSelected'])
  },
  methods: {
    ...mapActions(['getSalesByProducts'])
  },
  created () {
    this.getSalesByProducts({dateStart: this.dateMinSelected, dateEnd: this.dateMaxSelected})
  },
  watch: {
