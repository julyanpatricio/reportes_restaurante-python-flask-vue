...
export default {
  ...
  created () {
    this.dateStart =
      this.dateMin && new Date(this.dateMin.replaceAll('-', '/'))
  ...
  watch: {
    ...
    dateEnd: function (newData, oldData) {
      ...
      if (newData.toDateString() !== oldData.toDateString()) {
        this.setDateMaxSelected({
          dateMaxSelected: format(newData, 'yyyy-MM-dd')
        })
      }
    },
    dateStart: function (newData, oldData) {
      ...
      if (newData.toDateString() !== oldData.toDateString()) {
        this.setDateMinSelected({
          dateMinSelected: format(newData, 'yyyy-MM-dd')
        })
      }
    }
  }
